...
export const login = async (req, res) => {
  ...
  //   jika password cocok
  req.session.userId = user.uuid_user;
  //   get data user
  ...
  res.status(200).json({
    ...
    dataUser: { uuid_user, name, email, role, accessToken },
  });
};
...
export const sessi = async (req, res) => {
  ...
  // mencari user berdasarkan uuid_user / userId
  const user = await Users.findOne({
    attributes: ["uuid_user", "name", "email", "role"],
    where: {
      uuid_user: req.session.userId,
    },
  });
